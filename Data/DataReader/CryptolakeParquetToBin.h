...
#ifndef CRYPTOLAKEPARQUETTOBIN_H
#define CRYPTOLAKEPARQUETTOBIN_H
...
public:
    ...
    CryptolakeParquetToBin(std::string parquet_root, std::string bin_root);

    // Convert every parquet file under parquet_root into a mirrored .bin tree under bin_root.
    void convert_all_to_bin() const;

    // Convert one (symbol, exchange, date) parquet into mirrored .bin output.
    void convert_to_bin(const std::string& symbol, const std::string& exchange, const std::string& date) const;
...
#endif //CRYPTOLAKEPARQUETTOBIN_H
